...
public class WhileEndStmnt extends Stmnt {
  ...
  protected void linkToSymbol(HashMap<Symbol, SymbolTableEntry> table) {
    table.forEach((s, ste) -> {
      this.cloneTable.put(new Symbol(s.getVarName(), s.getTypeName()), 
...
//      System.out.println("ERROR ! Cannot resolve the while end statement !!");
    ...
    this.methodTable.table.forEach((s, ste) -> {
      SymbolTableEntry t = null;
      for (Entry<Symbol, SymbolTableEntry> e : this.cloneTable.entrySet()) {
        if (e.getKey().getVarName() == s.getVarName() && e.getKey().getTypeName() == s.getTypeName()) {
          t = e.getValue();
        }
      }
      if (t == ste) {
//        System.out.println("NOOOOOOOOOOOOOOOOOOOO");
      ...
      if (t == null) {
//        System.out.println("ERROR ! Cannot find the symbol in while resolve !!");
        return ;
      }
//      t.print();
    ...
    if(this.isDone) {
//      System.out.println("No change found in while loop !! " + this.cloneTable.size());
    }
